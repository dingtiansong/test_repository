...
class getFeatureVector(object):
    ...
    def fvector(self,sentence,factor0):
        index=factor0
        #tWord = factor0
        aa8=self.getIsDFirst(index, sentence, self.vType)
        aa9=self.getIsDLeft(index, sentence, self.verbTag)
        aa10=self.getIsDRight(index, sentence, self.verbTag)
        aa11=self.getIsExistVerb(index, sentence, self.verbTag)
        aa12=self.getIsFirstWord(index, sentence)
        aa13=self.getIsFourTag(index, sentence, self.vType)
        aa14=self.getIsLastWord(index, sentence)
        aa15=self.getIsPLeftD(index, sentence, self.nounTag, self.verbTag)
        aa16=self.getIsSerialRight(index, sentence, self.serialTag)
        aa17=self.getWordID(sentence[index], self.vWordList)
        aa18=self.getWordTags(sentence[index], self.vWordList)
    ...
    def getIsFirstWord(self, wordIndex, sentence):
        try:
            if wordIndex == 0:
                tmp = 1
            else:
    ...
    def getIsLastWord(self, wordIndex, sentence):
        try:
            if sentence[wordIndex] == sentence[-1]:
                tmp = 1
            else:
                tmp = 0
        except Exception as e:
    ...
    def getIsDLeft(self, wordIndex, sentence, verbTag):
        try:
            tmp = 0
    ...
    def getIsDRight(self, wordIndex, sentence, verbTag):
        try:
            tmp = 0
            #wordIndex = sentence.index(tWord)
    ...
    def getIsSerialRight(self, wordIndex, sentence, serialTag):
        try:
            tmp = 0
            #wordIndex = sentence.index(tWord)
    ...
    # 词    当前词及其前后各一个词    
    # L_V    当前词之前的句子中是否有动词    
    def getIsExistVerb(self, wordIndex, sentence, verbTag):
        try:
            tmp = 0
            #wordIndex = sentence.index(tWord)
            for i in sentence[:wordIndex]:
    ...
    def getIsDFirst(self, wordIndex, sentence, vType):
        try:
            tmp = 0
            #wordIndex = sentence.index(tWord)
            vTypeDict = {}
    ...
    def getIsPLeftD(self, wordIndex, sentence, nounTag, verbTag):
        try:
            tmp = 0
            #wordIndex = sentence.index(tWord)
    ...
    def getIsFourTag(self, wordIndex, sentence, vType):
        try:
            tmp = 0
            #wordIndex = sentence.index(tWord)
            tDict = {}
