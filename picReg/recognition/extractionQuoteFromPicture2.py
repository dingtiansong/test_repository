...
class cvPredictiveText(object):
    ...
    def __init__(self,tPath = 'D:/picreg/trainData/imageData.pkl',historypath='D:/picreg/trainData/historydata.pkl'):
        ...
        historydata=pickle.load(history_file)      
...
#                 n = nLength
    ...
    def getReadImg1(self,gray,historydata,minValue = 0,mLength=20,nLength = 80,minDist = 2):
        imgMin = self.getMinPic(gray,minValue)
        if len(imgMin) > 0:
            imgLength = 1
            for i in imgMin.shape:
            ...
            if imgLength > 1 and imgLength <= mLength*nLength:
                gray1 = self.getNormalizePic(imgMin,mLength,nLength).reshape(-1,mLength*nLength).astype(np.float32)        
                predictword=self.predictWord(gray1,historydata)
        else:
            return ''
        return predictword
