...
class cutTablePicture(object):
    ...
    #矩形框切割：
    def getCutColumn(self, edges, xList, yList, minLength = 13):
        xListNew = xList
        yListNew = yList
...
class cvPredictiveText(object):
    ...
    def __init__(self,tPath = 'D:/picreg/trainData/imageData.pkl',historypath='D:/historydata.pkl'):
        if tPath and historypath:
            self.dataPath = tPath
            self.historydatapath=historypath
        history_file=open('D:/historydata.pkl', 'rb')
    ...
    def convertToBinary(self,image):
        ...
        x = np.array(gray)
        wordindex=x.sum()/255
        wordvector=x.reshape((169,-1),order='F').T
...
#                 middovector=dovector[41:221]
                middovector=dovector[(cutpart*13+1):((14-cutpart)*13+1)]
...
#                     print wordindex
                if wordindex0<30 and flag1==((14-2*cutpart)*13):
#                     print wordindex0
                    if targetindex[j] not in wordindex and targetindex[j] !=[]:
                            wordindex.append(targetindex[j])
                else:
                    if flag1>((14-2*cutpart)*13-2):
                        if targetindex[j] not in wordindex and targetindex[j] !=[]:
                            wordindex.append(targetindex[j])
#                     print wordindex
        return wordindex        
...
#                 print '可能预测词：',fdist1.keys()[1],'            预测概率为：',fdist1.freq(fdist1.keys()[1])  
    ...
    def saveHistoryData(self,picpath,standwordpath,picnum):
        ...
        dataMatrix,wordlist,wordindex0=self.readPic(picpath,startnum,picnum)
...
#         print dictMerged['1020']
    ...
    def getNormalizePic(self,gray,mLength=13,nLength = 13):
        m,n = gray.shape
        if m > 13:
            print "error:m = %s"%(m)
            return np.array([])
...
#                 n = nLength
    ...
    def getReadImg(self,gray,minValue = 0,mLength=13,nLength = 13,minDist = 2):
        imgMin = self.getMinPic(gray,minValue)
        print imgMin
        if len(imgMin) > 0:
            imgLength = 1
